...
{
    public class CreateKirimSPDHOCommandHandler : ICommandHandler<CreateKirimSPDHOCommand>
    {
        ...
        public CreateKirimSPDHOCommandHandler(IUnitOfWork<FinanceContext> uow,
        ...
        public async Task Handle(CreateKirimSPDHOCommand command, CancellationToken cancellationToken)
        {
            try
            {

                var dokumenSPD = new List<DokumenSPD>();

                var dataKaryawan = await _approvalSPDQueries.GetInfoKaryawan(command.id_ms_login);
                var dataToKirim = await _approvalSPDQueries.GetToKirim(command.id_ms_login, dataKaryawan.Bagian, dataKaryawan.Divisi, dataKaryawan.PT);

                command.dokKey = command.dokKey.Distinct();

                foreach (var itemKey in command.dokKey)
                {
                    var singleData = dataToKirim.Where(x => x.DokKey == itemKey).FirstOrDefault();
                    dokumenSPD.Add(singleData);
                }
                ...
                foreach (var item in dokumenSPD)
                {
